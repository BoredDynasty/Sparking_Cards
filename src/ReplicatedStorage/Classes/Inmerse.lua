				...
				if Result then
					...
					if PlayingSound:GetAttribute("UsePitch") then
						if PlayingSound:GetAttribute("UsePitch") == true then
							PlayingSound.PitchFX.Octave =
								Random.new()
									:NextNumber(PlayingSound:GetAttribute("Min"), PlayingSound:GetAttribute("Max"))
						end
					end
					...
					if Settings.FootprintMaterials[Result.Instance.Material] then
						...
						local FootPrintResult =
							workspace:Raycast(FootPrintOrigin, FootPrintDirection * Settings.FloorDistance, Params)
						...
						if FootPrintResult then
							...
							NewCFrame = CFrame.lookAt(
								FootPrintResult.Position,
								FootPrintResult.Position + FootPrintResult.Normal
...
function module:PlayFaceAnimation() end
