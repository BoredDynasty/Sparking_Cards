...
local function CheckForListLayout(target, verbose)
	for _, d in pairs(target:GetChildren()) do
		...
		elseif d:IsA("UIGridLayout") then
			if verbose then
				warn("SWAGTEXT >> ", target, "contains UIGridLayout. This may cause issues with word formatting.")
			end
			return true
		end
	end
	if verbose then
		warn("SWAGTEXT >> ", target, "does not contain UIListLayout or UIGridLayout. One will be created.")
	end
	return false
end
...
local function TextAppear(target, mode)
	if mode then
		...
		if string.match(mode, "rotate") then
			target.Rotation = 30
			TS:Create(target, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Rotation = 0 })
				:Play()
		...
		elseif string.match(mode, "shake") then
			...
			TS:Create(target, TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), { Rotation = 0 })
				:Play()
		elseif string.match(mode, "rotultra") then
			target.Rotation = 360
			TS:Create(target, TweenInfo.new(0.7, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Rotation = 0 })
				:Play()
		end
	end
end
...
local commands = {
	["<red>"] = function()
		return {
			...
			rainbow = state[#state].rainbow,
			shake = state[#state].shake,
			warp = state[#state].warp,
...
function swagtext.AnimateText(str, location, delayTime, font, mode, wordMode, textSize, ahori, avert, extra, verbose)
	...
	if not CheckForListLayout(location, verbose) then
		...
		if ahori then
			layout.HorizontalAlignment = ahori
		end
		if avert then
			layout.VerticalAlignment = avert
		end
	end
