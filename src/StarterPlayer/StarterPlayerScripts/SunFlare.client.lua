-- DO NOT CHANGE THIS SCRIPT OR THE WHOLE MODEL WILL BREAK!
-- Scripted // Modified by 3UL4
...
local function GetSunPosition()
	local screenPoint = camera:WorldToScreenPoint(camera.CoordinateFrame.p + lighting:GetSunDirection())
...
local function IsObstructed(ignore)
	return workspace:FindPartOnRay(Ray.new(camera.CoordinateFrame.p, lighting:GetSunDirection()*900), ignore) ~= nil
end
...
local function OnCharacterAdded(character)
	...
	do --get valid ScreenGui
		...
		if not screenGui then screenGui = Instance.new("ScreenGui", playerGui) end
	end
	screenGui.IgnoreGuiInset = true
	--create washout effect
	local brightScreen = Instance.new("Frame", screenGui)
	...
	for i = 1, #FLARE_DIST do
		local flare = Instance.new("ImageLabel", screenGui)
	...
	--create sun flare
	local sunFlare = Instance.new("ImageLabel", screenGui)
	...
	runService:BindToRenderStep("SunFlareReposition", Enum.RenderPriority.Last.Value, function()
		...
		local isClear = not IsObstructed((root.Position+Vector3.new(0, 1.5, 0)-camera.CoordinateFrame.p).magnitude < 1.1 and character or nil) --ignores character when zoomed in
		...
		for i = 1, #flares do
			...
			flares[i].Position = UDim2.new(SUN_OFFSET.x, thisPosition.x-thisSize.x/2, SUN_OFFSET.y, thisPosition.y-thisSize.y/2)
		...
		sunFlare.Position = UDim2.new(SUN_OFFSET.x, sunPosition.x-sunFlareSize.x/2, SUN_OFFSET.y, sunPosition.y-sunFlareSize.y/2)
		sunFlare.Visible = isInFront
		
		--blur effect
		local cosAngle = camera.CoordinateFrame.lookVector:Dot(lighting:GetSunDirection())
		brightScreen.Transparency = 1-realMod + brightness(cosAngle)*realMod
	end)
end

repeat wait() until player.Character --fix for laggy CharacterAdded stuff
